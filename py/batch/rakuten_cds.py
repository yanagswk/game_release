    ...
    with get_my_chrome(playwright) as chrome:
        ...
        with chrome.new_page() as page:
            ...
            for n in range(cd_list.count()):
                ...
                cds_info["title"] = Common.get_title(title_selector.inner_text()) if title_selector.is_visible() else ""
                
                # ジャンル
                genre_selector = page.locator("#topicPath > dd > a:nth-child(3)")
                cds_info["genre"] = genre_selector.inner_text() if genre_selector.is_visible() else ""
                
                # ジャンル詳細
                genre_detail_selector = page.locator("#topicPath > dd > a:nth-child(4)")
                cds_info["genre_detail"] = genre_detail_selector.inner_text() if genre_detail_selector.is_visible() else ""

                # cdのアーティスト
                author_selector = page.locator(".productInfo:has-text(\"アーティスト\") .categoryValue")
                cds_info["author"] = author_selector.inner_text() if author_selector.is_visible() else ""

                # cdの関連作品
                relation_item_selector = page.locator(".productInfo:has-text(\"関連作品\") .categoryValue")
                cds_info["relation_item"] = relation_item_selector.inner_text() if relation_item_selector.is_visible() else ""

                # cdの発売元
                selling_agency_selector = page.locator(".productInfo:has-text(\"発売元\") .categoryValue")
                cds_info["selling_agency"] = selling_agency_selector.inner_text() if selling_agency_selector.is_visible() else ""

                # cdの販売元
                distributor_selector = page.locator(".productInfo:has-text(\"販売元\") .categoryValue")
                cds_info["distributor"] = distributor_selector.inner_text() if distributor_selector.is_visible() else ""

                # cdのディスク枚数
                disc_count_selector = page.locator(".productInfo:has-text(\"ディスク枚数\") .categoryValue")
                cds_info["disc_count"]  = disc_count_selector.inner_text() if disc_count_selector.is_visible() else ""

                # cdの曲数
                music_count_selector = page.locator(".productInfo:has-text(\"総曲数\") .categoryValue")
                cds_info["music_count"]  = music_count_selector.inner_text() if music_count_selector.is_visible() else ""

                # cdの収録時間
                record_time_selector = page.locator(".productInfo:has-text(\"収録時間\") .categoryValue")
                cds_info["record_time"]  = record_time_selector.inner_text() if record_time_selector.is_visible() else ""

                # cdの品番
                cd_number_selector = page.locator(".productInfo:has-text(\"品番\") .categoryValue")
                cds_info["cd_number"]  = cd_number_selector.inner_text() if cd_number_selector.is_visible() else ""

                # cdのJAN
                jan_selector = page.locator(".productInfo:has-text(\"JAN\") .categoryValue")
                cds_info["jan"]  = jan_selector.inner_text() if jan_selector.is_visible() else ""

                # cdのインストアコード
                in_store_code_selector = page.locator(".productInfo:has-text(\"インストアコード\") .categoryValue")
                cds_info["in_store_code"]  = in_store_code_selector.inner_text() if in_store_code_selector.is_visible() else ""

                # 本の発売日
                release_date_selector = page.locator(".productInfo:has-text(\"発売日\") .categoryValue")
                cds_info["release_date"]  = Common.format_release_date(release_date_selector.inner_text()) if release_date_selector.is_visible() else ""
                ...
                # gifの場合はスキップする
                cds_info["image_url"] = f"https:{image_url}" if image_url not in 'gif' else ""

                # 商品説明
                description_selector = page.locator("#editArea2")
                cds_info["description"]  = description_selector.inner_text() if description_selector.is_visible() else ""
                ...
                if not len(number_list) == 1:
                    ...
                    print(detail_page)
                    continue
                cds_info["rakuten_affiliate_url"] = f"https://hb.afl.rakuten.co.jp/ichiba/2cd5b0ae.6303a4de.2cd5b0af.dbaafed3/?pc=https%3A%2F%2Fitem.rakuten.co.jp%2Fbook%2F{number_list[0]}%2F&link_type=hybrid_url&ut=eyJwYWdlIjoiaXRlbSIsInR5cGUiOiJoeWJyaWRfdXJsIiwic2l6ZSI6IjI0MHgyNDAiLCJuYW0iOjEsIm5hbXAiOiJyaWdodCIsImNvbSI6MSwiY29tcCI6ImRvd24iLCJwcmljZSI6MCwiYm9yIjoxLCJjb2wiOjEsImJidG4iOjEsInByb2QiOjAsImFtcCI6ZmFsc2V9"
                
                print(cds_info)
                cds_data.append(cds_info)
            ...
            # ページごとにインサート
            releasedModel = ReleasedModel()
            releasedModel.insert_cds(cds_data)
